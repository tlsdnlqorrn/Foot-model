...
if __name__ == '__main__':
    ...
    path = 'C:/Users/shinwi/Desktop/FootModel/new.mp4'
    ...
    while True:
        ...
        if w < h:
            ...
            if rppg_switch:
                ppg1, bpm1, light_val1, cr_va1l, cb_val1 = rppg_1.process(
                    frame[y + int(round(h / 7 * 5)):y + h, x: x + int(round(w / 3))])
                ppg2, bpm2, light_val2, cr_val2, cb_val2 = rppg_2.process(
                    frame[y + int(round(h / 7 * 5)):y + h, x + int(round(w / 3)):x + int(round(w / 2))])
                ppg3, bpm3, light_val3, cr_val3, cb_val3 = rppg_3.process(
                    frame[y + int(round(h / 7 * 5)):y + h, x + int(round(w / 2)):x + int(round(w / 3 * 2))])
                ppg4, bpm4, light_val4, cr_val4, cb_val4 = rppg_4.process(
                    frame[y + int(round(h / 7 * 5)):y + h, x + int(round(w / 3 * 2)):x + int(round(w / 6 * 5))])
                ppg5, bpm5, light_val5, cr_val5, cb_val5 = rppg_5.process(
                    frame[y + int(round(h / 7 * 5)):y + h, x + int(round(w / 6 * 5)):x + w])
                ppg_all, bpm_all, light_val_all, cr_val_all, cb_val_all = rppg_all.process(frame[y:y + h, x:x + w])
                ...
                cv2.putText(ppg_screen1, '%03d' % round(bpm1), (ppg_screen1.shape[1] - 230, ppg_screen1.shape[0] - 50),
                            cv2.FONT_HERSHEY_SIMPLEX, 3, (0, 0, 255), 3)
                cv2.putText(ppg_screen2, '%03d' % round(bpm2), (ppg_screen2.shape[1] - 230, ppg_screen2.shape[0] - 50),
                            cv2.FONT_HERSHEY_SIMPLEX, 3, (0, 0, 255), 3)
                cv2.putText(ppg_screen3, '%03d' % round(bpm3), (ppg_screen3.shape[1] - 230, ppg_screen3.shape[0] - 50),
                            cv2.FONT_HERSHEY_SIMPLEX, 3, (0, 0, 255), 3)
                cv2.putText(ppg_screen4, '%03d' % round(bpm4), (ppg_screen4.shape[1] - 230, ppg_screen4.shape[0] - 50),
                            cv2.FONT_HERSHEY_SIMPLEX, 3, (0, 0, 255), 3)
                cv2.putText(ppg_screen5, '%03d' % round(bpm5), (ppg_screen5.shape[1] - 230, ppg_screen5.shape[0] - 50),
                            cv2.FONT_HERSHEY_SIMPLEX, 3, (0, 0, 255), 3)
                cv2.putText(ppg_screen_all, '%03d' % round(bpm_all),
                            (ppg_screen_all.shape[1] - 230, ppg_screen_all.shape[0] - 50), cv2.FONT_HERSHEY_SIMPLEX, 3,
                            (0, 0, 255), 3)

        else:
            img = cv2.rectangle(img, (x + round(w/7*5), y), (x + w, y + round(h/6)), (0, 0, 255), 2)
            img = cv2.rectangle(img, (x + round(w/7*5), y + round(h/6)), (x + w, y + round(h/3)), (0, 0, 255), 2)
            img = cv2.rectangle(img, (x + round(w/7*5), y + round(h/3)), (x + w, y + round(h/2)), (0, 0, 255), 2)
            img = cv2.rectangle(img, (x + round(w/7*5), y + round(h/2)), (x + w, y + round(h/3*2)), (0, 0, 255), 2)
            img = cv2.rectangle(img, (x + round(w/7*5), y + round(h/3*2)), (x + w, y + h), (0, 0, 255), 2)
            ...
            if rppg_switch:
                ppg1, bpm1, light_val1, cr_va1l, cb_val1 = rppg_1.process(frame[y:y + int(round(h/6)), x + round(w/7*5):x + w])
                ppg2, bpm2, light_val2, cr_val2, cb_val2 = rppg_2.process(frame[y + int(round(h/6)):y + int(round(h/3)), x + round(w/7*5):x + w])
                ppg3, bpm3, light_val3, cr_val3, cb_val3 = rppg_3.process(frame[y + int(round(h/3)):y + int(round(h/2)), x + round(w/7*5):x + w])
                ppg4, bpm4, light_val4, cr_val4, cb_val4 = rppg_4.process(frame[y + int(round(h/2)):y + int(round(h/3*2)), x + round(w/7*5):x + w])
                ppg5, bpm5, light_val5, cr_val5, cb_val5 = rppg_5.process(frame[y + int(round(h/3*2)):y + h, x + round(w/7*5):x + w])
                ppg_all, bpm_all, light_val_all, cr_val_all, cb_val_all = rppg_all.process(frame[y:y+h, x:x+w])
