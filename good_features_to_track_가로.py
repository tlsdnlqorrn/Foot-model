...
if __name__ == '__main__':
    ...
    while True:
        ...
        corners_1 = cv2.goodFeaturesToTrack(global_result_1, 1000, 0.01, 5, blockSize=5, useHarrisDetector=False)
        corners_2 = cv2.goodFeaturesToTrack(global_result_2, 1000, 0.01, 5, blockSize=5, useHarrisDetector=False)
        corners_1=corners_1[0].sort()
        for i in corners_1:
            center=tuple(i[0])
            # print(center)
            if global_mask_1[int(center[1])][int(center[0])]==0 and center[0]+180>370:
                cv2.line(frame, (int(center[0]+180),int(center[1])+60),(int(center[0]+180),int(center[1]+60)),(0, 0, 255), 10)

        for i in corners_2:
            center=tuple(i[0])
            # print(center)
            if global_mask_2[int(center[1])][int(center[0])]==0 and center[0]+180>370:
                cv2.line(frame, (int(center[0]+180),int(center[1]+300)),(int(center[0]+180),int(center[1]+300)),(0, 0, 255), 10)
